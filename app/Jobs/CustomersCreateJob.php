...
class CustomersCreateJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        // Sync this customer to connected stores
        $this->syncCustomerToConnectedStores();
    }

    /**
     * Sync the customer to connected stores.
     *
     * @return void
     */
    protected function syncCustomerToConnectedStores()
    {
        ...
        if ($storeConnection) {
            ...
            foreach ($connectedShops as $connectedShop) {
                // Create a new job to sync this customer to the connected shop
                SyncCustomerJob::dispatch($this->data, $sourceShopDomain, $connectedShop->shop_domain);
            }
        }
    }
}